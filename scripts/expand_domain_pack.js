...
function renderDomainDocs(pack, projectDir, dryRun) {
  ...
  const docs = new Map([
    ['docs/specs/domain-model.md', [
      ...
      '## Bounded Contexts',
      '',
      renderMarkdownTable(['ID', 'Name', 'Type', 'Responsibility', 'Aggregates'], boundedContextRows),
      '',
      '## Aggregates',
      '',
    ...
    ['docs/specs/use-cases.md', [
      '# Use Cases',
      '',
      renderMarkdownTable(['ID', 'Use Case', 'Actor', 'Requirement', 'Command/Query', 'Aggregate', 'Emits', 'Status'], useCaseRows),
      ''
    ].join('\n')],
...
function renderTraceability(pack, projectDir, generatedScenarios, dryRun) {
  ...
  for (const scenario of generatedScenarios) {
    ...
    if (mode === 'rich') {
      ...
      const eventRefs = asArray(scenario.events).length > 0
