...
| `pack lint` rejects a pack that worked before | A new schema version added required fields. | Run `npx specgate@latest pack lint …` against the latest CLI to see the actual error. |
| `npx specgate …` hangs the first time | npm is resolving the package. | Subsequent runs are cached; pin the version (e.g. `@0.1.0-beta.3`) in CI. |
...
Still stuck? Open an issue with the output of `npx specgate@latest validate --help` plus the failing command. The [Comparisons doc](comparisons.md) lists migration paths from `spec-kit`, Cursor rules, Aider conventions, and plain READMEs if the answer is "this tool isn't the right fit".
