...
| `validate`                       | Check structure, traceability and Gherkin coverage. `--strict-tdd` also fails the build when a `REQ` lacks its `.feature`, its executable test, or its row in `traceability.md`. Every failure prints a `file:line` and a concrete fix command. `--fix` auto-repairs mechanical issues first; `--json` emits a structured result. |
...
| `expand`                         | Low-level: apply a domain pack onto a project. **Prefer `specops add`** (writes `.specops.lock`); direct use prints a steer. `specops add`/`sync` call this internally.          |
