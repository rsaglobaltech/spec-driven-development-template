  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case "--engine": {
        ...
        if (engine !== "sdk" && engine !== "scripted") {
          throw new CliError(`Unknown engine: ${engine}.`, "Expected: sdk | scripted.");
        }
        opts.engine = engine;
        break;
      }
...
export function helpText(): string {
  return `
  csda-agent — interactive agentic CLI for spec-driven projects
  ...
  OPTIONS
    -p, --print                 Non-interactive: run once, print, exit.
        ...
        --engine <sdk|scripted> Engine behind the boundary (default: sdk).
