...
| A5 | Docs por niveles L1–L4 | ✅ **Hecho** | README: tabla de niveles L1–L4 + quickstart brownfield (`adopt`) antes que greenfield; wizard documentado. how-to.md: receta 0 "Adopt SDD on an existing repository" + guía "Where do I start?" por nivel. |
...
| B3 | Wrapper Maven/Gradle | ✅ **Hecho** | `packages/maven-plugin` (`com.rsaglobaltech:csda-maven-plugin`): goals `csda:validate` (falla build, bind a verify), `csda:plan`, `csda:doctor`. Gradle: `packages/gradle-plugin` (plugin `com.rsaglobaltech.csda`, tasks `csdaValidate`/`csdaPlan`/`csdaDoctor`, `csdaValidate` cableada a `check`). Ambos comparten launcher auto npx→docker (imagen mirroreable, lógica en `CsdaLauncher`). Verificado real: `mvn csda:validate` verde y build FAILURE al romper gate; `gradle build` verde, plugin aplica en consumer y `check`→`csdaValidate`. 4 tests del launcher Gradle. Jobs CI con setup-java (Maven + Gradle). |
