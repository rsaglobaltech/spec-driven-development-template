...
| H14 | **27 de los 28 escenarios de los packs curados tienen cero pasos para Cucumber.** Las plantillas escriben `GIVEN`/`WHEN`/`THEN` en mayúsculas y las palabras clave de Gherkin son sensibles a mayúsculas: el parser real las absorbe como descripción. Un escenario vacío **pasa** (`1 scenario (1 passed) · 0 steps · exit 0`), y `pack lint --strict` lo aprueba porque su mini-parser es insensible a mayúsculas | Anotado el 2026-08-22, reproducido con el Cucumber instalado. Es H1 con otro disfraz: la señal de recompensa aprueba sin ejecutar nada, y afecta a todo proyecto sembrado con `specops add`. Detalle y reproducción en `valoracion-bdd-gherkin-era-agentes.md` §2.1; propuesta F2 |
