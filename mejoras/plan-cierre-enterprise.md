...
| C10-02 | `[ ]` | Paridad MCP ↔ superficie del CLI ([#138](https://github.com/rsaglobaltech/specgate/issues/138)) | Las 25 ausentes, por orden de valor: `status`, `req add/link`, el ciclo `change` completo, `specops add/diff --as-change`, `harness run`, `report`. Sin esto, «MCP como interfaz primaria» es una frase |
...
**0.8.0 y 0.8.1 cortadas el 2026-09-01** (D19). D17 cerrada como decisión, pero la release **no está completa**: Docker (`ghcr.io/rsaglobaltech/specgate:0.8.1`) y GitHub Packages (`@rsaglobaltech/specgate`) publicados; **npm público pendiente** — ver D21.
...
| C10-02…05 — paridad MCP (7 herramientas de 32 comandos) | [#138](https://github.com/rsaglobaltech/specgate/issues/138) | No |
...
| 2026-09-01 | El publish a npm de 0.8.1 falla con **404 sobre el scope**, y queda esperando acción en npmjs.com (D23) | `404 Not Found - PUT /@rsaglobaltech%2fspecgate`. Ya no es el bloqueo de nombre —eso lo resolvió el scope— sino que npm responde 404 en vez de 403 cuando la cuenta autenticada no puede escribir en un scope, para no revelar si existe. La org `rsaglobaltech` está creada, pero la cuenta del token (`rtexido`, maintainer de `create-spec-driven-app`) no consta como miembro con permiso de escritura. No requiere versión nueva: basta relanzar el job de `publish-npm.yml` sobre `v0.8.1`. **Mientras tanto la portada documenta un `npx` que no resuelve**, que es el mismo fallo que el rename existía para cerrar |
