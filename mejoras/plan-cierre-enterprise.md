...
| H25 | **La puerta protege la especificación pero no su propio comando.** `DEFAULT_PROTECTED_PATHS` cubre `spec.md`, `AI_RULES.md`, `features/**` y `docs/specs/**` — el contrato. No cubre el fichero donde vive el comando que la puerta ejecuta. Reproducido con un agente stub que **no escribe una línea de implementación** y solo cambia `"test"` a `echo 'all good'` en `package.json`, sobre una suite en rojo: `✅ REQ-001 pass (1 attempt)`. El que no consigue pasar el test puede debilitar el test, que es A1 exactamente, un nivel más arriba | Encontrado en el spike de #105 por accidente: el comando de test del fixture estaba roto y el agente real lo arregló para pasar. El arreglo era correcto; el que **pudiera** hacerlo no. Repro: `mejoras/spike-105-evidence/repro-gate-command-unprotected.sh`. **Sin arreglar** — el arreglo obvio (proteger `package.json`) bloquearía añadir una dependencia, que es trabajo legítimo, así que necesita diseño |
| H26 | **Un escenario declarado que nadie prueba pasa todas las puertas.** `--strict-scenarios` mide la *calidad* del Gherkin —When, Then, tres pasos—, nunca su *cobertura*, y la matriz nombra un `Scenario ID` por fila para un fichero que puede contener cinco. Reproducido sin agente: feature con tres escenarios, tests que cubren dos, fila en `Implemented`, y `validate` con `--strict-tdd --strict-scenarios --strict-links` sale 0 | Encontrado en el spike de #105 forzando un fallo: el agente escribió tests para cuatro de cinco escenarios y **omitió el que no podía satisfacer**, y la puerta aprobó. Coincide con lo que `nimbus-billing` reportó por otra vía: la web afirma «scenarios must actually execute». Repro: `mejoras/spike-105-evidence/repro-uncovered-scenario.sh`. **Sin arreglar** — casar id contra artefacto de test es heurística y hacerlo obligatorio rompe a todo proyecto con features de varios escenarios |
